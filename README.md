...
| **`dump-balances`** | Export all non-zero accounts from the current state trie to a file `$prefix.txt`. The output is a two-column list (address and ETH balance with 6 decimal places), sorted in descending order by balance. (--datadir /path/to/your/datadir --out /path/to/output/ --verbose). 
                                                                                       |
